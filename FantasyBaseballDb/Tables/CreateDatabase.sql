...
CREATE TABLE BattingStint (
  ...
  OnBasePercentage as case when (AtBats+Walks+HitByPitch+SacrificeFlies) = 0 then 0 else (Hits+Walks+HitByPitch)/(Cast(AtBats as float)+Walks+HitByPitch+SacrificeFlies) end,
  SluggingPercentage as case when AtBats = 0 then 0	else (Hits+Doubles+(2*Triples)+(3*HomeRuns))/(Cast(AtBats as float)) end,
  OnBasePlusSlugging as case when AtBats = 0 or (AtBats+Walks+HitByPitch+SacrificeFlies) = 0 then 0 else ((Hits+Walks+HitByPitch)/(Cast(AtBats as float)+Walks+HitByPitch+SacrificeFlies)) + ((Cast(Hits as float)+Doubles+(2*Triples)+(3*HomeRuns))/(AtBats)) end,
...
CREATE TABLE PitchingStint (
  ...
  WalksAndHitsPerInningsPitched as case when InningsPitchedOuts = 0 then 0 else (Walks+Hits)/(Cast(InningsPitchedOuts as float))/3 end,
  WalksAndHitsPerInningsPitchedPlusEarnedRunAverage as case when InningsPitchedOuts = 0 then 0 else (Walks+Hits)/(Cast(InningsPitchedOuts as float))/3 + EarnedRunAverage end,
